...
const MyShopPage: React.FC = () => {
  ...
  const handleToggleSold = async (productId: string, currentStatus: boolean) => {
    ...
    // If unmarking as sold, just update the product status
    try {
      const { error } = await supabase
        .from('products')
        .update({ 
      ...
      setProducts(products.map(p => 
        p.id === productId 
          ? { ...p, is_sold: false, status: 'available' } 
          : p
      ))
    } catch (error) {
  ...
  const handleBuyerSelected = async (buyer: Buyer) => {
    if (!productToSell || !user) return

    try {
      // Optimistically update the UI
      setProducts(products.map(p => 
        p.id === productToSell.id ? { ...p, is_sold: true, status: 'sold' } : p
      ))
      ...
      alert(`Product sold to ${buyer.display_name || buyer.email}!`)
    } catch (error) {
      console.error('Error in sale process:', error)
      // Revert optimistic update if needed
      setProducts(products.map(p => 
        p.id === productToSell.id ? { ...p, is_sold: false, status: 'available' } : p
      ))
    ...
    return (
      <div className="min-h-screen bg-gray-100">
        ...
        <BuyerSelectionModal
          ...
          productName={productToSell?.title || ''}
